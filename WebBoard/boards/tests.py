...
class HomeTests(TestCase):
    ...
    def test_home_url_contains_link_to_topic_page(self):
...
class NewTopicTests(TestCase):
    ...
    def test_new_topics_url_resolves_board_topics_view(self):
        view = resolve('/boards/1/new/')
        self.assertEquals(view.func, new_topic)

    def test_new_topic_contains_link_to_homepage(self):
        new_topics_url = reverse('new_topic', kwargs={
            'pk': 1
        })
        board_topics_url = reverse('board_topics', kwargs={
            'pk': 1
